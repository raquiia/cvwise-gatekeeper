...
export const useCandidateScore = (candidate: CandidateData) => {
  ...
  useEffect(() => {
    ...
    // Priorité 1 : utiliser le score IA si disponible (maintenant avec cache optimisé)
    ...
    if (aiScore.score !== null && !aiScore.isLoading && !aiScore.error) {
      const contextualScore: ContextualScore = {
        ...
        matchContext: aiScore.isJobSpecific ? 
          `Score IA de correspondance${aiScore.source ? ` (${aiScore.source === 'database' ? 'BDD' : aiScore.source === 'fresh_calculation' ? 'Nouveau' : 'Cache'})` : ''}` : 
          `Score IA de complétude${aiScore.source ? ` (${aiScore.source === 'database' ? 'BDD' : aiScore.source === 'fresh_calculation' ? 'Nouveau' : 'Cache'})` : ''}`,
        details: {
    ...
    // Priorité 2 : utiliser le système de scoring classique
    ...
    // Si aucun score n'est disponible, calculer avec l'IA en priorité (maintenant avec cache optimisé)
    const loadScore = async () => {
      try {
        setIsLoading(true);
        setError(null);
        
        // Essayer d'abord avec l'IA (qui vérifiera automatiquement le cache)
