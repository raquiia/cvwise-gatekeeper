...
BEGIN
  ...
  -- Return the saved score with explicit table prefixes to avoid ambiguity
  RETURN QUERY
  SELECT 
    scores.id,
    scores.candidate_id,
    ...
    scores.created_at,
    scores.updated_at
  FROM ai_candidate_scores scores
  WHERE scores.candidate_id = p_candidate_id
    AND scores.job_offer_id IS NOT DISTINCT FROM p_job_offer_id
    AND scores.user_id = v_user_id;
