...
DROP FUNCTION IF EXISTS public.get_candidate_by_id_bypassing_rls(uuid);

-- Supprimer les tables complexes liées aux scores IA
...
DROP TABLE IF EXISTS public.candidate_job_matching_scores CASCADE;
...
-- Recréer la fonction RPC avec le bon type de retour incluant les colonnes AI
...
AS $function$
BEGIN
  -- Vérifier que le candidat appartient à l'utilisateur actuel
  IF NOT EXISTS (
  ...
  -- Retourner toutes les données du candidat, y compris les colonnes AI
  RETURN QUERY
  SELECT 
    c.id,
    c.user_id,
    ...
    c.last_updated_at,
    -- Colonnes AI directement depuis la table candidates
    c.ai_score,
    c.ai_explanation,
    c.ai_breakdown,
    c.ai_strengths,
